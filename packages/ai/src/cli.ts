...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @elynyx/ai <command> [provider]
...
Examples:
  npx @elynyx/ai login              # interactive provider selection
  npx @elynyx/ai login anthropic    # login to specific provider
  npx @elynyx/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @elynyx/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @elynyx/ai --help' for usage`);
	process.exit(1);
}
