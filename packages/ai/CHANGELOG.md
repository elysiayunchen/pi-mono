...
- Lazy-load built-in provider modules and root provider wrappers so importing `@elynyx/ai` no longer eagerly loads provider SDKs, significantly reducing base startup cost without changing dependency installation footprint ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@elynyx/ai/oauth` package exports to point directly at built `dist` files, avoiding broken TypeScript resolution through unpublished wrapper targets ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level package entry. Import OAuth login/refresh functions from `@elynyx/ai/oauth` instead of `@elynyx/ai` ([#1814](https://github.com/badlogic/pi-mono/issues/1814))
...
- Re-exported TypeBox `Type`, `Static`, and `TSchema` from `@elynyx/ai` to match documentation and avoid duplicate TypeBox type identity issues in pnpm setups ([#1338](https://github.com/badlogic/pi-mono/issues/1338))
...
- **Agent API moved**: All agent functionality (`agentLoop`, `agentLoopContinue`, `AgentContext`, `AgentEvent`, `AgentTool`, `AgentToolResult`, etc.) has moved to `@elynyx/agent-core`. Import from that package instead of `@elynyx/ai`.
