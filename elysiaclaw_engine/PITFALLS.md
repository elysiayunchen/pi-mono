...
10. **代码+测试存在 ≠ 完成**：必须有生产路径实跑 + 端到端验证（坑 #85）
...
*记录截至 2026-06-07，坑 #94。下次遇到新坑从 #95 开始追加。*

---

### #85 — 设计-实现鸿沟：代码+测试存在 ≠ 完成（系统性坑）

**现象**: 多个模块在文档中标注 "✅ IMPLEMENTED" 或 "✅ 已完成"，但 PARTICIPANT-CONTINUITY-ARCHITECTURE.md §七 诚实审查（AS1-AS7）揭露真相——`executeRotation` 是死代码、自动轮换只是提示文字、CONSOLIDATE 未接、安全点硬约束写死 0/false、注入预算器存在但未喂给运行时、B4 RECALL 方向反了、整套模块未部署从未端到端跑过。

**根因**: 项目缺乏"代码存在 ≠ 完成"的 Definition of Done。PITFALLS #64 提到了 DoD 但未将"生产路径实跑 + 端到端验证"纳入硬性标准。测试覆盖给人"已完成"的错觉——测试覆盖的是模块内部逻辑，不是生产调用链。

**影响**: 全项目最严重的系统性问题。设计文档与实际运行状态严重脱节，新协作者/Agent 误信 "✅" 标记后做出错误决策。

**解决**:
1. 全文清理所有 "✅ IMPLEMENTED" 标记，改为三级：`📝 代码存在` / `🔄 已接线待验证` / `✅ 生产验证通过`
2. DoD 硬性增加：(a) 生产路径有调用者（grep 证实）(b) 端到端实跑验证 (c) 部署后验证
3. W0 闭环应作为下一 Sprint 的唯一目标，不再并行其他工作

**预防**:
- 信 ✅ 前先 `grep -rn funcName src | grep -v test`，零命中即死代码
- 测试覆盖 ≠ 接入运行时（与用户画像写路径死代码、computeInjectionBudget 架空同构）
- 每次标注"完成"时必须回答：谁在生产路径调用了它？端到端跑过吗？
...
| #5 | #45 | session 文件路径 |
| #6 | #46 | 工具注册遗漏(grep) |
| #7 | #47 | 工具注册遗漏(plan_mode) |
| #8 | #48 | YAML 缩进 |
| #9 | #49 | Context Entropy |
| #10 | #50 | Vibe vs Environment |
| #11 | #51 | 工具注册四层 |
| #12 | #52 | pnpm/npm 混用 |
| #13 | #53 | elysiaclaw 构建部署 |
| #14 | #54 | Bot/TUI 工具路径 |
| #15 | #55 | web_search API 密钥 |
| #37 | #43 | (与旧 #3 合并) |
| #38 | #38 | TypeScript 类型错误(未重编号) |
| #39 | #39 | system prompt 构建(未重编号) |

> 注：#1-#2 和 #16-#42 未重编号。#43-#59 为重编号后的连续序列。#60+ 为新增坑号。
