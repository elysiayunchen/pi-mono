...
**源码参考**: `/home/elysia/pi-mono/claude-code-source-code-main/src/tools/BashTool/` (完整目录)

**当前差距**:
- 缺少 `isConcurrencySafe` / `isReadOnly` / `isDestructive` 声明
- 缺少 `run_in_background` 参数
- 缺少 `getToolUseSummary` / `getActivityDescription`
- 缺少 `toAutoClassifierInput`（安全分类）
- 缺少 `preparePermissionMatcher`（命令模式匹配，如 `git *`）

**步骤**:
1. 读 Claude Code `BashTool/BashTool.ts` 完整源码（当前未成功读取，需重试）
2. 读 Claude Code `BashTool/prompt.ts`
3. 在 bashToolDefinition 中添加能力声明（依赖 Task 0 接口扩展）：
   - `isConcurrencySafe: () => false`
   - `isReadOnly: () => false`
   - `isDestructive: () => true`
4. 添加 `getToolUseSummary` / `getActivityDescription`
5. 添加 `toAutoClassifierInput`（返回命令字符串供安全分类）
6. 添加 `preparePermissionMatcher`（支持 `git *` 级别的命令模式匹配）
7. 评估 `run_in_background` 参数：
   - 如果 ElysiaClaw 的 background-runner.ts 已有此能力，只需包装参数
   - 如果没有，标记为后续任务
8. `npm run build` + `npm run check`

**完成标准**:
- [ ] 能力声明字段全部填充
- [ ] `getToolUseSummary` 返回命令摘要
- [ ] `toAutoClassifierInput` 返回命令供分类器使用
- [ ] `preparePermissionMatcher` 支持通配符匹配
