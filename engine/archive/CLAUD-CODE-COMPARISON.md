...
- 输出结构化: `outputSchema?`
- 路径提取: `getPath?`, `preparePermissionMatcher?`
- UI 增强: `getToolUseSummary?`, `getActivityDescription?`
- 安全分类: `toAutoClassifierInput?`

`wrapToolDefinition()` 为所有新字段提供安全默认值，现有 31 个工具无需改动。

### GrepTool 增强 (Task 1, 2026-04-10)

新增参数: `output_mode` (content/files_with_matches/count), `-A` (after context), `-B` (before context), `type` (file type filter), `offset` (pagination), `multiline` (跨行匹配), `count` 模式 (`--count`), mtime 排序, `isConcurrencySafe`/`isReadOnly` 能力声明, VCS 目录补全 (`.jj`, `.sl`)。
...
- **工具数量**: 31 个导出工具
- **接口增强**: Task 0 已扩展至 18 字段 (对标 Claude Code Tool 25+ 方法)
...
### Phase 1: DTS 类型错误修复 + delegate_code_task 验证（1 Sprint）
1. 修复 6 个 DTS 类型错误（compaction.ts / attempt.ts 等），消除 pnpm build 阻塞
2. Telegram 端到端验证 delegate_code_task 功能（Code Mode 已废弃）
3. 修复 dynamic-provider 测试回归根因

### Phase 2: Web 工具（1 Sprint，无前置依赖，用户直接感知）
1. **WebFetchTool** — HTTP 获取，参考 Claude Code WebFetchTool（代码已存在，完成 Bot 路径注册）
...
2. isConcurrencySafe 能力声明（Task 0 已完成，可直接使用）
3. 并行执行的权限检查适配

### Phase 4: MCP 集成（2-3 Sprint，最复杂）
1. MCPConnectionManager — 连接管理（stdio/sse/http/ws）
2. MCPTool — MCP 工具包装器
3. ListMcpResourcesTool / ReadMcpResourceTool

### Phase 5: 压缩优化（1 Sprint）
